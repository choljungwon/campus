...
//   }
	...
	@GetMapping("/adminreadItem")
	public void adminreadItem(@RequestParam("itemCode") String itemCode, Model model) {
		log.info("adminread Item");
		model.addAttribute("item", adminService.adminreadItem(itemCode));
	}

	@GetMapping("/adminshoplist")
	public void adminshoplist(Model model, Criteria cri, Authentication authentication, Locale locale) {
		GetAuth.getAuth(authentication, model);
		//log.info("data:"+ToStringBuilder.reflectionToString(cri));
		log.info("data:"+cri.getTypeArr());
		List<ItemVO> data = adminService.adminshoplist(cri);
		model.addAttribute("adminshoplist", data);

		int total = data.get(0).getCnt();
		log.info("total:"+total);
		model.addAttribute("pageMaker", new PageDTO(cri, total));
	}

//   @GetMapping("/adminshoplist")
//   public void adminshoplist() {
//	   
//   }
